...
export default function MealDetail() {
  ...
  const { id, isTrainerPlan } = useLocalSearchParams();
  ...
  useEffect(() => {
    async function loadData() {
      ...
      const tableName = isTrainerPlan === 'true' ? 'trainer_meal_plan_day_meals' : 'customer_meal_plan_day_meals';
      const idField = isTrainerPlan === 'true' ? 'trainerMealPlanDayMealId' : 'customerMealPlanDayMealId';
    ...
    <View className="flex-1 bg-[#0A0A0A] pb-28">
      ...
      >
        ...
        <View className="px-5 pt-5">
          <Text className="text-white text-2xl font-semibold mb-3">{meal.mealName}</Text>

          <View className="bg-[#1A2E00] self-start px-2 py-1 rounded mb-4">
            <Text className="text-[#C4EF00] text-[10px] font-semibold uppercase">{meal.mealType}</Text>
          </View>
          ...
          <View className="bg-[#141414] border border-[#222222] rounded-[16px] p-4 flex-row justify-between mb-8">
            <View className="items-center">
              <Text className="text-white text-lg font-semibold mb-1">{meal.calories}</Text>
              <Text className="text-[#8E8E93] text-[10px]">kcal</Text>
            </View>
            <View className="items-center">
              <Text className="text-white text-lg font-semibold mb-1">{meal.protein}g</Text>
              <Text className="text-[#8E8E93] text-[10px]">Protein</Text>
            </View>
            <View className="items-center">
              <Text className="text-white text-lg font-semibold mb-1">{meal.carbs}g</Text>
              <Text className="text-[#8E8E93] text-[10px]">Carbs</Text>
            </View>
            <View className="items-center">
              <Text className="text-white text-lg font-semibold mb-1">{meal.fat}g</Text>
              <Text className="text-[#8E8E93] text-[10px]">Fat</Text>
            </View>
            <View className="items-center">
              <Text className="text-white text-lg font-semibold mb-1">{fiberAmount}g</Text>
              <Text className="text-[#8E8E93] text-[10px]">Fiber</Text>
            </View>
            <View className="items-center">
              <Text className="text-white text-lg font-semibold mb-1">{meal.weight || totalWeight}g</Text>
          ...
          {isTrainerPlan !== 'true' && (
            <View className="mt-8">
              <Text className="text-white text-lg font-semibold mb-4">Swap with other options</Text>
              ...
              <View className="gap-y-4">
                    ...
                    <View className="flex-1 pr-4">
                      ...
                      <View className="flex-row items-center">
                        ...
                        <View className="flex-row items-center gap-x-3">
                          ...
                          <Text className="text-white text-[10px] font-semibold"><Text className="text-white">Fi</Text> {alt.scaledFiber}g</Text>
                        </View>
                      </View>
                    </View>
                ...
                {visibleLimit < alternatives.length && (
                  ...
                  >
                    <Text className="text-[#C4EF00] text-sm font-semibold">Load More</Text>
                  </Pressable>
                )}
