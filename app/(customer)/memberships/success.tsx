      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, padding: 20, paddingTop: 60, paddingBottom: (insets.bottom || 0) + 140, justifyContent: 'center' }} showsVerticalScrollIndicator={false}>
        {Platform.OS === 'ios' ? (
          <View className="flex-1 items-center justify-center mt-20">
            <View className="w-24 h-24 rounded-full bg-[#CCFF00] items-center justify-center mb-6 shadow-[0_0_40px_rgba(204,255,0,0.4)] relative">
              <View className="absolute inset-0 rounded-full bg-[#CCFF00] opacity-30 scale-125" />
              <Check size={48} color="#000" weight="bold" />
            </View>
            <Text className="text-white text-2xl font-bold mb-3 text-center">Membership Updated</Text>
            <Text className="text-[#9CA3AF] text-sm text-center px-4 leading-6 mb-12">
              Your membership status has been successfully updated by your gym provider.
            </Text>
            <Pressable
              onPress={() => router.replace('/(customer)/memberships')}
              className="bg-[#CCFF00] rounded-2xl py-4 px-8 flex-row items-center justify-center active:opacity-80 w-full"
            >
              <ArrowLeft size={20} color="#000" weight="bold" style={{ marginRight: 8 }} />
