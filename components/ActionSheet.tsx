...
import { View, Pressable, Modal, Platform, ScrollView } from 'react-native';
            ...
            <View className="w-full max-w-[310px] bg-[#1E1E1E] rounded-3xl overflow-hidden border border-[#2E2E2E] shadow-2xl">
              {title && (
                <View className="pt-5 pb-3 items-center justify-center border-b border-[#2C2C2C]/40">
                  <Text className="text-[#A1A1AA] text-xs font-semibold uppercase tracking-wider">{title}</Text>
                </View>
              )}

              <ScrollView className="px-2 py-1.5" style={{ maxHeight: 350 }}>
                {options.map((opt, idx) => (
                  <Pressable
                    key={idx}
                    onPress={() => {
