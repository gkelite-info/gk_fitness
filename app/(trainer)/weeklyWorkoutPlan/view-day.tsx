...
export default function ViewDay() {
  ...
  const { data: dayData, isLoading: isLoadingDay } = useTrainerWorkoutPlanDayById(dayId);
  const [page, setPage] = useState(1);
  const [limit] = useState(10);
  const [accumulatedExercises, setAccumulatedExercises] = useState<any[]>([]);

  const { data: eData, isLoading: isLoadingExercises, isFetching: isFetchingExercises } = usePaginatedTrainerWorkoutPlanDayExercises(dayId, page, limit);
  ...
  const openVideo = (exerciseName: string, videoUrl?: string | null) => {
    ...
    if (videoUrl) {
      localVideoSource = { uri: videoUrl };
    } else if (normalizedTitle.includes('incline') && normalizedTitle.includes('dumb')) {
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            return (
              <Pressable
                onPress={() => openVideo(item.exerciseName || item.name, fullVideoUrl)}
      ...
      >
        <View className="flex-1 bg-black/95 justify-center items-center">
          ...
          <View className="w-full h-80 bg-black">
            {selectedVideo && (
              (videoTitle?.toLowerCase().includes('chin up') || videoTitle?.toLowerCase().includes('chin-up') || videoTitle?.toLowerCase().includes('chinups') || (typeof selectedVideo === 'object' && selectedVideo.uri?.toLowerCase().endsWith('.gif'))) ? (
                <Image
