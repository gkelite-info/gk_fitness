...
import { useTrainerWorkoutPlans } from '@/hooks/trainerWorkoutPlans/useTrainerWorkoutPlans';
import { useTrainerWeeklyPlan } from '@/hooks/trainerWorkoutPlans/useTrainerWeeklyPlan';
import { useSwapTrainerWorkoutDays } from '@/hooks/trainerWorkoutPlans/useMutateTrainerWorkoutPlan';
import { toast } from '@/lib/toast';
import { CustomRefreshControl } from '@/components/CustomRefreshControl';
import { useQueryClient } from '@tanstack/react-query';

export default function SwapDay() {
  const { dayId, customerId } = useLocalSearchParams<{ dayId: string, customerId: string }>();
  ...
  const handleRefresh = async () => {
    ...
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ['trainerWorkoutPlans', customerId] }),
      queryClient.invalidateQueries({ queryKey: ['trainerWeeklyPlan', customerId] })
    ]);
    setIsManualRefreshing(false);
  };

  const { data: plans } = useTrainerWorkoutPlans(customerId);
  const activePlanId = plans?.find((p: any) => p.isActive)?.planId || null;
  const { data: loadedPlanDays, isLoading: isQueryLoading } = useTrainerWeeklyPlan(customerId);
  const swapMutation = useSwapTrainerWorkoutDays();
