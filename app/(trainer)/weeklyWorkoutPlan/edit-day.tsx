...
export default function EditWorkoutDay() {
  const { day, customerId } = useLocalSearchParams<{ day: string, customerId: string }>();
  ...
  const { data: dayById, isLoading: isLoadingDayById } = useTrainerWorkoutPlanDayById(isUUID ? day : null);
  const { data: weeklyPlan, isLoading: isLoadingWeeklyPlan } = useTrainerWeeklyPlan(!isUUID ? customerId : null);
  ...
  const { data: fetchedExercises, isLoading: isLoadingExercises, refetch: refetchExercises } = useTrainerWorkoutPlanDayExercises(currentPlanDayId);
  ...
  const onRefresh = async () => {
    ...
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: ['customerWeeklyPlan', customerId] })
    ]);
  ...
  const saveExercisesMutation = useSaveTrainerWorkoutDayExercises();
  const makeRestMutation = useMakeTrainerRestDay();
