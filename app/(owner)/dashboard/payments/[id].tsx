...
import { mockPayments } from '@/constants/paymentsMockData';
...
const PaymentMethodBadge = ({ method }: { method: string }) => {
  return (
    <View className="border border-[#CCF200] px-2 py-0.5 rounded-sm items-center justify-center">
      <Text className="text-[10px] font-bold text-[#CCF200] uppercase tracking-wider">{method}</Text>
    </View>
  );
};
...
export default function PaymentDetailsScreen() {
  ...
  const payment = mockPayments.find((p) => p.id === id) || mockPayments[0]; // fallback for demo
  ...
  const handleDownload = () => {
    Alert.alert('Download Started', 'The receipt is downloading...');
  };

  const handleShare = () => {
    Alert.alert('Share', 'Sharing receipt options...');
  };
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 140 }}>
        ...
        <View className="items-center mt-6 mb-8">
          <View className="w-20 h-20 rounded-full border border-[#4d5c00] items-center justify-center mb-4">
            <Text className="text-[#CCF200] text-2xl font-bold">{getInitials(payment.memberName)}</Text>
          </View>
          <Text className="text-white text-2xl font-bold mb-2">{payment.memberName}</Text>
          <View className="bg-[#141414] border border-[#27272A] rounded-full flex-row items-center px-3 py-1.5">
            ...
            <Text className="text-[#888888] text-[10px] font-bold tracking-wider ml-2 uppercase">MEMBER ID: {payment.memberId}</Text>
          </View>
        </View>
        ...
        <View className="bg-[#121214] border border-[#27272A] rounded-3xl p-5 mb-5">
          ...
          <View className="flex-row items-center justify-between mb-6">
            ...
            <Text className="text-white text-sm font-bold">₹{payment.amount.toLocaleString()}</Text>
          </View>
        ...
        <TouchableOpacity onPress={handleDownload} className="bg-[#121214] border border-[#27272A] rounded-2xl p-4 flex-row items-center justify-between mb-3 active:opacity-70">
          <View className="flex-row items-center gap-4">
            ...
            <View>
              <Text className="text-white text-sm font-bold mb-0.5">Payment Receipt</Text>
        ...
        <TouchableOpacity onPress={handleShare} className="bg-[#121214] border border-[#27272A] rounded-2xl p-4 flex-row items-center justify-between active:opacity-70">
          <View className="flex-row items-center gap-3">
            ...
            <Text className="text-[#CCF200] text-sm font-bold ml-1">Share Payment Receipt</Text>
          </View>
